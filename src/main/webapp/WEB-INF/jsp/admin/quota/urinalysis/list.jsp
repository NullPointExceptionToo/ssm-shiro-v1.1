             ...
             <a><cite><i class="layui-icon" style="line-height:25px">&#xe68e  </i>首页</cite></a>
              <a><cite>检查单列表</cite></a>
                ...
                <a class="layui-btn layui-btn-mini" style="line-height:1.0em;margin-top:1px;float:right"
                   href="${pageContext.request.contextPath}/admin/quota/urinalysis/index" title="刷新">
                    <i class="layui-icon" style="line-height:25px">&#x1002</i></a>
            </div>
        </div>

        <a class="layui-btn layui-btn-normal" role="button"
               style="margin-right: 10px;float: right" href="${pageContext.request.contextPath}/admin/quota/urinalysis/add">添加检查单</a>
        <div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
            <table class="layui-table"
                   lay-data="{height: 'full-110',even: true,url:'${pageContext.request.contextPath}/admin/quota/urinalysis/list',limits:[10,30,50,100], limit: 10,page:true,id:'id'}"
                   lay-filter=urinalysis>
                <thead>
                <tr>
                    <th lay-data="{field:'id',align:'center', width:100, sort: true}">检查单ID</th>
                    <th lay-data="{field:'protein',align:'center', width:150, sort: true, templet: timeTpl1}">蛋白质[-]</th>
                    <th lay-data="{field:'red_cell',align:'center', width:150, sort: true, templet: timeTpl2}">红细胞[0-22.7]</th>
                    <th lay-data="{field:'white_cell',align:'center', width:150, sort: true, templet: timeTpl3}">白细胞[0-16.9]</th>
...
<script>
    layui.use(['table', 'element', 'jquery'], function () {
        ...
        //监听工具条
        table.on('tool(urinalysis)', function (obj) {
            var data = obj.data;
            ...
            if (obj.event === 'delete') {
                layer.confirm('确定要删除该检查单吗？宝', {
                    ...
                    layer.closeAll();
                    $.post("${pageContext.request.contextPath}/admin/quota/urinalysis/del", {id: data.id}, function (data) {
                        if ("success"==data) {
                            //如果删除成功,则刷新页面
            ...
            if (obj.event === 'detail') {
                window.location.href = '${pageContext.request.contextPath}/admin/quota/urinalysis/detail/' + data.id;
            }
            if (obj.event === 'update') {
                window.location.href = '${pageContext.request.contextPath}/admin/quota/urinalysis/update/' + data.id;
            }
        });
        //弹出层
        var msg = "${msg}";
