             ...
             <a><cite><i class="layui-icon" style="line-height:25px">&#xe68e  </i>首页</cite></a>
              <a><cite>检查单列表</cite></a>
                ...
                <a class="layui-btn layui-btn-mini" style="line-height:1.0em;margin-top:1px;float:right"
                   href="${pageContext.request.contextPath}/admin/quota/routine/index" title="刷新">
                    <i class="layui-icon" style="line-height:25px">&#x1002</i></a>
            </div>
        </div>

        <a class="layui-btn layui-btn-normal" role="button"
               style="margin-right: 10px;float: right" href="${pageContext.request.contextPath}/admin/quota/routine/add">添加检查单</a>
        <div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
            <table class="layui-table"
                   lay-data="{height: 'full-110',even: true,url:'${pageContext.request.contextPath}/admin/quota/routine/list',limits:[10,30,50,100], limit: 10,page:true,id:'id'}"
                   lay-filter=routine>
                <thead>
                <tr>
                    <th lay-data="{field:'id',align:'center', width:100, sort: true}">检查单ID</th>
                    <th lay-data="{field:'white_count',align:'center', width:150, sort: true, templet: timeTpl1}">白细胞计数[4-10]</th>
                    <th lay-data="{field:'mid_cell',align:'center', width:150, sort: true, templet: timeTpl2}">中性粒细胞[2-7]</th>
                    <th lay-data="{field:'lb_cell',align:'center', width:150, sort: true, templet: timeTpl3}">淋巴细胞[0.8-4.0]</th>
                    <th lay-data="{field:'hemoglobin',align:'center', width:150, sort: true, templet: timeTpl4}">血红蛋白[113-151]</th>
                    <th lay-data="{field:'red_cell_jy',align:'center', width:150, sort: true, templet: timeTpl5}">红细胞积压[33.5-45.0]</th>
...
<script>
    layui.use(['table', 'element', 'jquery'], function () {
        ...
        //监听工具条
        table.on('tool(routine)', function (obj) {
            var data = obj.data;
            ...
            if (obj.event === 'delete') {
                layer.confirm('确定要删除该检查单吗？宝', {
                    ...
                    layer.closeAll();
                    $.post("${pageContext.request.contextPath}/admin/quota/routine/del", {id: data.id}, function (data) {
                        if ("success"==data) {
                            //如果删除成功,则刷新页面
            ...
            if (obj.event === 'detail') {
                window.location.href = '${pageContext.request.contextPath}/admin/quota/routine/detail/' + data.id;
            }
            if (obj.event === 'update') {
                window.location.href = '${pageContext.request.contextPath}/admin/quota/routine/update/' + data.id;
            }
        });
        //弹出层
        var msg = "${msg}";
